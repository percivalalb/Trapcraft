...
public abstract class IPacket {
	...
	public Packet getPacket() {
		if(FMLCommonHandler.instance().getEffectiveSide().isClient())
			return new C17PacketCustomPayload(Properties.CHANNEL_NAME, this.getPacketBytes(true));
		else
			return new S3FPacketCustomPayload(Properties.CHANNEL_NAME, this.getPacketBytes(false));
	}
	...
	private byte[] getPacketBytes(boolean isClient) {
		try {
	    	...
	    	if(isClient)
	    		dos.writeUTF(TrapcraftMod.proxy.getClientPlayer().getCommandSenderName());
			this.write(dos);
